        ...
        private static object ParseValue(Type type, string value)
        {
            ...
            else if (type.IsPrimitive)
            {
                ...
                catch (InvalidCastException)
                {
					...
					try {
						return Convert.ChangeType(value[0], type, CultureInfo.InvariantCulture); // Boolean, Byte, SByte, Int16, UInt16, Int32, UInt32, Int64, UInt64, IntPtr, UIntPtr, Char, Double, and Single.
					}
					catch (InvalidCastException) {
						return null;
					}
            ...
            else if (type.IsGenericType && type.GetGenericTypeDefinition() == typeof(List<>))
            {
				...
				// Handle as name or ARGB
				if (value[0] == '"' && value[value.Length - 1] == '"')
					return Color.FromName(ParseString(value));
				else if (value[0] == '{' && value[value.Length - 1] == '}') {
					var dict = (Dictionary<string, object>)ParseUnkownValue(value);
					return Color.FromArgb(
						Convert.ToInt32(dict["A"]),
						Convert.ToInt32(dict["R"]),
						Convert.ToInt32(dict["G"]),
